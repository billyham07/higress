...
func applyResolvedHeaders(config ModelRouterConfig, entry *ModelEntry) {
	...
	// These headers are a trusted handoff between the model-router, key-auth and
	// ai-credits stages.  Client copies were removed before model resolution;
	// the values below come only from the immutable published registry entry.
	setTrustedContextHeader("x-ai-credits-canonical-model", entry.CanonicalID)
	setTrustedContextHeader("x-ai-credits-release-id", entry.ReleaseID)
	setTrustedContextHeader("x-ai-credits-binding-id", entry.BindingID)
	setTrustedContextHeader("x-ai-credits-price-version", entry.PriceVersion)
}

func setTrustedContextHeader(name, value string) {
