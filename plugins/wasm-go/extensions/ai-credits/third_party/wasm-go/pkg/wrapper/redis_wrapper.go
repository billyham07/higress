...
type RedisClusterClient[C Cluster] struct {
	cluster C
	// clusterName is the complete hostcall cluster identity produced by Init.
	// RedisInit accepts buffering/database options as query parameters on that
	// identity. Dispatching with only the base C.ClusterName() would therefore
	// keep using the old host-side client after a reconnect that changes those
	// options. Keep the exact initialized identity for every command.
	clusterName    string
	ready          bool
...
type redisDispatchCluster struct {
	name string
}

func (c redisDispatchCluster) ClusterName() string {
	return c.name
}

func (c redisDispatchCluster) HostName() string {
	return c.name
}
...
func (c *RedisClusterClient[C]) Command(cmds []interface{}, callback RedisResponseCallback) error {
	...
	return redisCallInternal(c.dispatchCluster(), respString(cmds), callback, &c.ready, c.checkReadyFunc)
}

func (c *RedisClusterClient[C]) dispatchCluster() redisDispatchCluster {
	name := c.clusterName
	if name == "" {
		name = c.cluster.ClusterName()
	}
	return redisDispatchCluster{name: name}
}
...
func (c *RedisClusterClient[C]) Eval(script string, numkeys int, keys, args []interface{}, callback RedisResponseCallback) error {
	...
	return redisCallInternal(c.dispatchCluster(), respString(params), callback, &c.ready, c.checkReadyFunc)
}
...
func (c *RedisClusterClient[C]) Del(key string, callback RedisResponseCallback) error {
	...
	return redisCallInternal(c.dispatchCluster(), respString(args), callback, &c.ready, c.checkReadyFunc)
}
