...
func onHttpRequestBody(ctx wrapper.HttpContext, config CreditsConfig, body []byte) types.Action {
	...
	if config.Registry != nil && config.Registry.Enabled {
		if config.Registry.publicationBlocked() {
			...
			if !resolved.OK {
				...
				releaseID = resolved.Entry.ReleaseID
				bindingID = resolved.Entry.BindingID
				priceVersion = resolved.Entry.PriceVersion
	...
	ctx.SetContext(ctxReleaseID, releaseID)
	ctx.SetContext(ctxBindingID, bindingID)
	ctx.SetContext(ctxPriceVersion, priceVersion)
	setTrustedCreditsHeader("x-ai-credits-canonical-model", canonical)
	setTrustedCreditsHeader("x-ai-credits-release-id", releaseID)
	setTrustedCreditsHeader("x-ai-credits-binding-id", bindingID)
	setTrustedCreditsHeader("x-ai-credits-price-version", priceVersion)
...
func handleAdmit(ctx wrapper.HttpContext, config CreditsConfig, body []byte, statusCode int, responseBody []byte) {
	...
	if parsed.CanonicalModel != "" {
		...
		setTrustedCreditsHeader("x-ai-credits-canonical-model", parsed.CanonicalModel)
	}
	if parsed.PriceVersion != "" {
		ctx.SetContext(ctxPriceVersion, parsed.PriceVersion)
		setTrustedCreditsHeader("x-ai-credits-price-version", parsed.PriceVersion)
	}
...
func settleRequest(ctx wrapper.HttpContext, status, reason string, usage UsagePayload, httpStatus int) SettleRequest {
	return SettleRequest{
		...
		ReleaseID:       stringFromCtx(ctx, ctxReleaseID),
		BindingID:       stringFromCtx(ctx, ctxBindingID),
		PriceVersion:    stringFromCtx(ctx, ctxPriceVersion),
		Status:          status,
...
func stripCreditsClientHeaders() {
	for _, name := range []string{
		...
		"x-ai-credits-release-id",
		"x-ai-credits-binding-id",
		"x-ai-credits-price-version",
		DefaultAuthHeader,
...
func setTrustedCreditsHeader(name, value string) {
