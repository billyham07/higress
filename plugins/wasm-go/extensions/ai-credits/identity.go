...
const (
	...
	errUnboundedEstimate     = "unbounded_estimate"
...
func parseModelEntry(item gjson.Result, index int) (*ModelEntry, error) {
	...
	entry := &ModelEntry{
		...
		ReleaseID:     strings.TrimSpace(item.Get("releaseId").String()),
		BindingID:     strings.TrimSpace(item.Get("bindingId").String()),
		PriceVersion:  strings.TrimSpace(item.Get("priceVersion").String()),
